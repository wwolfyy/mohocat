 ...
 * rotates, but on mobile the image is rendered 90° CW, so north points **right**
 * there and the whole compass is rotated 90° CW to match (`rotate-90`); on
 * desktop north is up. Drawn as the classic compass needle (a slim diamond, red
 * north / light south) with a small "N", white + drop-shadow so it reads over
 * the satellite image without a bulky chip.
 *
 ...
 */
export default function Compass({ isMobile = false }: { isMobile?: boolean }) {
  return (
    <div
      className={
      ...
      role="img"
      aria-label={isMobile ? '북쪽은 오른쪽입니다' : '북쪽은 위쪽입니다'}
      title={isMobile ? '북쪽은 오른쪽' : '북쪽은 위쪽'}
    >
      {/* Counter-rotate the label on mobile so the letter stays upright while
          the needle (and the rest of the compass) is rotated to point right. */}
      <span className={'text-[11px] font-bold text-white' + (isMobile ? ' -rotate-90' : '')}>
        N
      </span>
