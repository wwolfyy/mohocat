            ...
            <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6 overflow-hidden">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Configure required permissions to access specific pages.</p>
                    </div>
                    ...
                    <table className="w-full text-sm text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                <th className="px-4 py-3 font-medium text-gray-500 uppercase tracking-wider w-1/4">Page / Resource</th>
                                {ALL_PERMISSIONS.map(perm => (
                                    <th key={perm} className="px-4 py-3 font-medium text-gray-900 text-center uppercase tracking-wider">
                                        {perm}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {RESOURCES.map(resource => (
                                <tr key={resource.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    {ALL_PERMISSIONS.map(permission => {
                                        ...
                                        return (
                                            <td key={`${resource.id}-${permission}`} className="px-4 py-3 text-center border-l border-gray-100">
                                                <input
                                                    type="checkbox"
