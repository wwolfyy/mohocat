...
export default function RolePermissionConfig() {
    ...
    const roles = Object.keys(config.roles).filter(r => ['admin', 'butler-ground', 'butler-internet', 'viewer'].includes(r));
                    ...
                    <table className="w-full text-sm text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                <th className="px-4 py-3 font-medium text-gray-500 uppercase tracking-wider w-1/4">Permission</th>
                                {roles.map(role => (
                                    <th key={role} className="px-4 py-3 font-medium text-gray-900 text-center uppercase tracking-wider w-1/6">
                                        {role === 'butler-ground' ? 'butler-ground' :
                                            role === 'butler-internet' ? 'butler-internet' :
                                                role}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {ALL_PERMISSIONS.map(permission => (
                                <tr key={permission} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-3 font-medium text-gray-700 font-mono">
                                        {permission}
                                    </td>
                                    {roles.map(role => {
