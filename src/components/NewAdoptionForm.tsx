...
import { getAdoptionService, getStorageService } from '@/services';
 ...
 * /pages/adoption. Adapted from NewAnnouncementForm; no 팝업(모달) toggle.
 */
const NewAdoptionForm = () => {
  ...
  const handleVideoFilesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      setVideoFiles([...videoFiles, ...Array.from(event.target.files)]);
    }
  };
  ...
  const uploadImages = async (files: File[]): Promise<string[]> => {
    const uploadPromises = files.map(async (file) => {
      const path = `adoption/images/${Date.now()}_${file.name}`;
  ...
  const uploadVideosToYouTube = async (files: File[]): Promise<string[]> => {
    const uploadPromises = files.map(async (file) => {
      ...
      formData.append('title', title || '입양홍보 동영상');
      formData.append('description', message || '입양홍보 동영상');
      formData.append('tags', '입양홍보');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adoptionService.createPost(postData);
      ...
      alert('입양홍보 글이 성공적으로 작성되었습니다!');

      router.push('/pages/adoption');
    } catch (error) {
      alert(
