...
import { User, UserCredential, browserLocalPersistence, browserSessionPersistence, setPersistence, onAuthStateChanged } from 'firebase/auth';
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    // Phone sign-in
    const signInWithPhoneNumber = useCallback(async (phoneNumber: string, appVerifier: any) => {
        clearErrors();
        try {
    ...
    const confirmPhoneLogin = useCallback(async (confirmationResult: any, verificationCode: string) => {
        clearErrors();
        try {
    ...
    const updatePhoneNumber = useCallback(async (verificationId: string, verificationCode: string) => {
        clearErrors();
        try {
    ...
    const reauthenticateWithType = useCallback(async (type: 'password' | 'phone', credentialData: any) => {
        clearErrors();
        try {
