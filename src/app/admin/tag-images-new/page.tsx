...
export default function TagImagesPage() {
  ...
  const batchDeleteImages = async () => {
    ...
    if (!confirm(`Are you sure you want to delete metadata for ${selectedImages.size} images? (Storage files will remain)`)) return;
    ...
    try {
      ...
      await imageService.batchDeleteImages(selectedIds);

      // Update local state
      setImages(images.filter(img => !selectedImages.has(img.id)));
      clearSelection();
      alert(`Successfully deleted ${selectedIds.length} images!`);
    } catch (err: any) {
