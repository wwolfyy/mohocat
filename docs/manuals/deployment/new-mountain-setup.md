...
| _(no `theme`)_         | ⚠️ **There is no per-mountain color setting** (removed 2026-08-05, owner decision). Every mountain uses the **same** palette, defined in `tailwind.config.js` alone. This block used to hold `primaryColor`/`secondaryColor`/`accentColor`; the latter two were never wired to anything, and a per-tenant primary was dropped because a mountain that picks its own colors is an administrative burden with no preview and no contrast check. **Do not add it back.** To change the brand color for the whole platform, edit `brand` in `tailwind.config.js`. |
...
- [ ] Header shows the tenant's `name`. 📌 **The colors are deliberately identical to every
      other mountain** — the CTA gradient is the shared brand yellow, not a per-tenant color.
