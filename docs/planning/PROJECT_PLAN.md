...
| **Colour / design tokens (2026-08-05)**        | `[x]` **done 2026-08-08**   | **§10u** — colour now has one source of truth (`tailwind.config.js`); **no new token file was created — the repo already designated one** (`design.md:9`, and `:292` forbids a `tokens.json`). 🔑 **Per-tenant theming is WITHDRAWN, superseding M8** — a mountain may not differ in colour (owner): no preview, no contrast check, and the config is baked, so each try is a redeploy. The `--color-primary` variable is **kept** (`theme('colors.brand.DEFAULT')`, build-resolved) because Tailwind tokens cannot reach `<style jsx global>` / third-party CSS; three hand-copied `#FACC15`s collapse to one. ~30 brand utilities adopted `brand`/`accent` (admin included — `design.md`'s "admin out of scope" line was stale, subsystem deleted in `d963d30`). ⚠️ **One user-visible change:** 급식현황's freshness ramp goes green→red ⇒ blue→red, lifting its worst contrast from **1.37:1 to 6.47:1** and dropping the colour-blindness-hostile pair; `feeding_spots` is seeded at last so the table has e2e cover. **Phase 4 (2026-08-06) closed the hygiene:** the last raw hexes leave `YouTubeAuthPanelNew` and `Compass` (`LeafletMountainMap:302` keeps its by decision — Leaflet writes an SVG **presentation attribute**, which takes neither a class nor `var()`), and **`design.md` §Colors now carries the global-palette decision**, which had been recorded everywhere except the design reference. **Open:** Phase 5, the unsized admin-vs-`design.md` audit D5 opened. Gates: tsc · unit 196 · smoke 39 · e2e 233/13/0. ✅ **Phase 5 CLOSED 2026-08-08 — the workstream is done** (`df132d0`). The admin CMS was measured against the **non-colour** halves of `design.md` for the first time (D5), and the audit **inverted its own premise**: spacing, elevation and language were already clean (zero arbitrary spacing values, only `shadow-sm/md/lg`, no English in any user-facing string), and **41 of 44** `alert`/`confirm` sites already routed through `ui/Modal`. The real gaps were **absences in `design.md`**, which was landing-first and had no admin tier. Fixed: five hand-rolled modal shells onto `ui/Modal` (gaining portal, scroll-lock, `role="dialog"`, click-outside, ESC); six admin `h1`s normalised with an **admin type tier** written into §Typography; **28** genuine buttons/cards off bare `rounded`; `text-md` (**not a Tailwind class** — it compiled to nothing) → `text-sm`; three native `alert()`s onto `useDialog`; one classless `h3` sized. ⚠️ **Three places the obvious fix was wrong:** the modals are **not** on `useDialog.confirm()` (two are destructive and §Modal requires a **red** button, which that API cannot render — the "conformance" fix would have removed the danger affordance); the playlist selector needed `closePlaylistSelector` extracted so the shell's new backdrop-click and ESC reset the ticked playlists as 취소 does; and **F3's approved fix rested on a false premise** — normalising admin's 73 bare `rounded` sites assumed the public UI met a spec admin missed, but measuring first showed the same usage there (38% of admin's radii, 23% of the public ones), so §Shapes gained **descriptive** rows for inputs/badges/checkboxes instead. 🔑 **Check the thing you are conforming to before conforming to it.** Gates: `tsc` 0 · smoke 39 · unit 196 · e2e 233/13/0. ⏳ `/admin/*` still unseen in a browser (auth-gated).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Work-tracking restructure (2026-08-08)**     | `[~]` **ACTIVE — next up**  | **Not a numbered §** — a redesign of how work is tracked, owner-initiated after this session made the cost concrete. ▶️ **This is the ACTIVE workstream (owner, 2026-08-08): application work is PAUSED and restarts once the new structure is in place** — tenancy T0 was asked for and then stood down behind it. Design + migration inventory: [`work-tracking-restructure-20260808.md`](./pending/work-tracking-restructure-20260808.md). 🔑 **Two problems, measured rather than asserted.** (1) The backlog is **not centralized** — open items split **5 in `BACKLOG.md` · 20 here · 9 in `HANDOFF.md`**, so the file named for the purpose holds under a sixth of it; this is the exact rot pattern `BACKLOG.md`'s own header warns about after the 2026-08-02 audit found seven stale claims. (2) **Decisions and rejected approaches have no home** — bugs go to `DEBUG_LOG`, changes to `FEATURE_MOD_LOG`, but "we investigated X and dropped it" lives only in hand-off prose, the least durable place for the entries that most prevent re-derivation. ⚠️ **The five docs total ~944 KB (~250k tokens) — they have outgrown their reader** (reading a fifth of `HANDOFF.md` cost 25k tokens in one session). **Agreed:** one row per item in a single store (`type` = task/decision/bug/change, `status` = open/in-progress/done/abandoned, backlink to a plan §), a **generated** `TASKS.md` view, and a much smaller hand-off. 🔴 **Open and blocking: the storage medium** — owner proposed SQLite for its structure; assistant argued structured text (NDJSON) on diffability, PR-reviewability and branch-merge grounds. Settle it in §4 of that doc first. 📌 **Until then the existing convention stands.** Adjacent: **this file is 320K — larger than `HANDOFF.md`** — from single table cells running to thousands of words (the multi-tenant row is one cell holding all of M1–M8); and `HANDOFF.md` needs a size policy (33 files already in `handoff/archive/`; the mechanism exists, it stopped being applied).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- 🔴 **OPEN (2026-08-08) — the work-tracking storage medium.** Blocks the restructure in
  [`work-tracking-restructure-20260808.md`](./pending/work-tracking-restructure-20260808.md) §4
  and nothing else. Owner proposed **SQLite** for its structure ("not for data volume"); the
  assistant argued for **structured text (NDJSON)** — same one-row-per-item shape, same schema
  enforcement, same partial loading via `grep`, but diffable, PR-reviewable, editor-readable,
  and **mergeable across branches**, which a `.db` is not. Both positions are written up there.
  📌 Either way the bridge exists: text can generate a `.db` on demand, or a `.db` can commit an
  NDJSON dump alongside. **Settle it and record the choice in that doc.**
