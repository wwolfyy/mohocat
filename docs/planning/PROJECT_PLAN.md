...
## 10u. 🎨 Colour has one source of truth; per-tenant theming is withdrawn (Phases 1–3 DONE 2026-08-05; 4–5 open)
...
- [ ] **U6 (plan Phase 4) — hygiene.** `YouTubeAuthPanelNew:114-127`'s status hex map
      (raw emerald/amber/red/gray-500); `LeafletMountainMap:302` + `Compass:33,35`; and
      **record in `design.md` §Colors that the palette is global**. ⚠️ **That last one is more
      than hygiene:** the "no per-tenant colour" decision currently lives in `AGENTS.md`, this
      doc and the plan — **but not in the design reference**, which is where a designer would
      look before proposing per-tenant theming again.
