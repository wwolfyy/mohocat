...
## 10n. ✅ Members author and edit their own 집사톡 / 급식현황 posts (BUILT 2026-08-03, NOT LIVE)
...
belongs to **§10p**.
Plan: [`member-post-authoring-20260802.md`](./pending/member-post-authoring-20260802.md).

---

## 10p. 🔴 Members can post on 집사톡 but cannot attach media (FIXED in tree 2026-08-03, awaiting deploy)
...
- [ ] **P8 — deploy:** rules → migration (`{added: 4, alreadyHeld: 3}`) → push.
      ⚠️ **Rules before the grant**, or a photo reaches Storage while its `cat_images` write
      is denied and swallowed — the post saves and the photo never reaches the album,
      silently.
...
Plan: [`member-media-upload-permissions-20260803.md`](./pending/member-media-upload-permissions-20260803.md).

---

## 10q. ✅ Authors may delete their own posts, and reply authors may edit + delete their replies (2026-08-04)
...
⚠️ **Ships with §10p** — same rules file, so the same `firebase deploy --only firestore:rules`
covers both. No migration: these grants ride on the existing `write-own-post-*` permissions.
