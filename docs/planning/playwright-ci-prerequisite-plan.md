...
- [ ] **🔬 S1 — Admin SDK against the emulators.** With
      `FIRESTORE_EMULATOR_HOST` / `FIREBASE_AUTH_EMULATOR_HOST` /
      `FIREBASE_STORAGE_EMULATOR_HOST` set and `initializeApp({ projectId: 'demo-mohocat' })`
      (no credential): verify (a) Firestore read/write, (b) Auth user create,
      (c) **`verifyIdToken` accepts an emulator-minted ID token** (the Admin SDK
      skips signature checks in emulator mode — confirm our version does), and
      (d) whether the existing `cert()` path in `firebase-admin.ts` must be
      bypassed or tolerates a dummy key. **Fallback:** a structurally-valid
      dummy service-account JSON (fake keypair) checked into `tests/e2e/fixtures/`.
- [ ] **🔬 S2 — `getSignedUrl` vs the Storage emulator.** Seed one file, run the
      asset script's exact call (`file.getSignedUrl({action:'read',…})` → axios
      GET). If the URL is wrong/unfetchable (the known rough edge), **fallback:**
      an emulator-gated branch in the script that uses `file.download()` /
      `file.createReadStream()` instead of signed-URL+axios (behavior-identical:
      bytes land in `public/images/…`).
- [ ] **🔬 S3 — `next/image` + emulator-hosted media.** Render one `<Image>`
      whose `src` is a Storage-emulator URL with a localhost `remotePatterns`
      entry added. If the optimizer chokes on the emulator's response headers,
      **fallback:** fixture Firestore docs point media URLs at
      `public/`-served fixture files (`/images/test-fixtures/…`) instead of
      emulator Storage — album/lightbox tests don't actually care where bytes
      come from. (Storage emulator then only serves the _build-time_ asset
      fetch, which S2 covers.)
- [ ] **🔬 S4 — client SDK + emulators through the real UI.** Boot `next dev`
      with WP2's wiring: email/password sign-in via the actual login page against
      the Auth emulator; one client-SDK Firestore write (any admin edit) to
      confirm the **repo `firestore.rules` load and `hasPermission` resolves**
      against seeded `role_permissions/role-config`. This proves the whole
      D1 premise end-to-end before investing in fixtures.

**Gate:** spike results recorded in this doc (1–3 lines each) before Phase B.
...
- [ ] devDeps: `@playwright/test`, `firebase-tools`.
- [ ] `firebase.json`: `emulators` block — `auth` (9099), `firestore` (8080),
...
- [ ] Storage emulator rules file (Storage emulator requires one; permissive
      read is fine for tests) + reference it from `firebase.json`.
- [ ] `.env.test` (committed — contains **only fake values**):
...
- [ ] After init, when `process.env.NEXT_PUBLIC_USE_FIREBASE_EMULATORS === 'true'`:
...
- [ ] Same flag **skips `getAnalytics`** (F1) — `analytics` exports `null`,
      which is already its SSR value, so no consumer changes.
- [ ] Guard is explicit `=== 'true'`; the var is never set in the Vercel
...
- [ ] Per S1's result: when the emulator env vars are present, initialize
...
- [ ] Verify every Admin-SDK consumer works in this mode: `cat-reads.ts`
...
- [ ] `FIREBASE_PROJECT_ID`: honor an env override (`FIREBASE_PROJECT_ID_OVERRIDE`
...
- [ ] Init: under emulator env vars, skip the `cert()`/local-file requirement
...
- [ ] Download path per S2: keep signed-URL+axios if it works against the
...
- [ ] **Cats (~6 docs):** stable doc ids (e.g. `test-cat-01`…); mix of 현재
...
- [ ] **Thumbnail images:** tiny (≤5 KB) placeholders named to satisfy F6 —
...
- [ ] **Posts:** 공지 ×2 (one with `[img]`/`[video]` link-tokens), 입양홍보 ×1,
...
- [ ] **Users + auth:** Auth-emulator users `admin@test.local` /
      ...
      `currentRole` set.
- [ ] **RBAC:** `role_permissions/role-config` granting admin all `manage-*`;
      `resource_permissions` map matching prod shape (Navigation reads it
      anonymously — main-plan 5.5 asserts its GET stays open).
- [ ] **about_content** doc (the about page + admin editor read it).
- [ ] **Media docs** (`cat_images` ×~6, `cat_videos` ×2): URLs per S3's
      outcome (emulator Storage or `public/` fixture files).
- [ ] Format: one JSON file per collection + an `images/` dir; ids/emails/
...
- [ ] Per S3: if fixture media stays on emulator Storage, add an env-gated
...
- [ ] **Leaflet ready signal (F11):** `data-testid="mountain-map"` on the map
...
- [ ] `playwright.config.ts`: two projects (D3), `baseURL`, `webServer`
      (`next start`, `reuseExistingServer: !CI`), `trace: 'on-first-retry'`,
      CI `retries: 2` / local `0`, `fullyParallel`.
- [ ] `tests/e2e/setup/global.setup.ts`: sign in admin + member through the
      real login UI against the Auth emulator → save `storageState` per role.
- [ ] Console-error watchdog fixture (shared `test` extension) — ready for
      Phase 2 of the main plan even if only the trivial spec uses it now.
- [ ] **Trivial spec** (`public/landing.smoke.spec.ts`): home page loads, map
...
- [ ] `tests/e2e/README.md`: how to run locally, fixture conventions, the
...
- [ ] Update main plan (mark its Phase 0+1 as delegated→done), PROJECT_PLAN §10 + §1 snapshot, FEATURE_MOD_LOG entry for the env-gated `src/` touches.
...
- [ ] Spikes S1–S4 resolved + recorded.
- [ ] Trivial spec green **locally** (full emulator loop, prod-mode serve).
- [ ] Trivial spec green **in GitHub Actions**, 3× consecutively, < 10 min job.
- [ ] Production build path verified untouched (`npm run build` against real
      Firebase succeeds; `vercel-build` unchanged; no new env vars in Vercel).
- [ ] Docs + logs updated (WP8).
