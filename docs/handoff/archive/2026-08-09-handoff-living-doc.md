> 📦 **Frozen 2026-08-09.** This is the living hand-off as it stood when its contents were
> migrated into the work registry; `docs/handoff/HANDOFF.md` is now a short current-state page.
> Every item and decision here exists as a record in [`work_tracking/`](../../../work_tracking/),
> each pinned to the commit it was read from.
>
> 📌 **Relative links were repointed by one level for the move into `archive/`; nothing else was
> touched.** The byte-exact original is `git show 2e07e3e:docs/handoff/HANDOFF.md`.
