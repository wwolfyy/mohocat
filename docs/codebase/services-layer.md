...
| Concrete implementations                   | `cat-service.ts`, `point-service.ts`, `image-service.ts`, `video-service.ts`, `post-service.ts`, `butler-talk-service.ts`, `announcement-service.ts`, `contact-service.ts`, `storage-service.ts`, `auth-service.ts`, `feeding-spots-service.ts`, `feeding-spots-admin-service.ts`, `basic-feeding-spots-service.ts`, `about-content-service.ts`, `permission-service.ts`, `media-albums.ts`, `youtube.ts`, `thumbnailPreloader.ts`, `role-assignment-service.ts` (used directly by `RoleManagement`/`PermissionDebug`, not via the factory) | One file per service. Most are `Firebase*Service implements IXxxService`. `feeding-spots-admin-service.ts` and `basic-feeding-spots-service.ts` use the Admin SDK directly (server-side); their inits read the service-account JSON from `config/firebase/`.                                                                                 |
...
- **`MIGRATION_EXAMPLE.ts` was removed (Phase 3A, 2026-06-27).** It was an unused
  reference snippet disguised as code; deleted.
- **`role-assignment-service.ts` is not exposed via the factory but is live.** It's
  imported directly by `RoleManagement.tsx` and `PermissionDebug.tsx` (`new
RoleAssignmentService()`). **Not dead** — keep it. (Optionally surface it via
  `index.ts` for consistency, but that's cosmetic.)
